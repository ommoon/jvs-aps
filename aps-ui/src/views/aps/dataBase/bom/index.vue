<template>
  <div class="basic-layout-box aps-dataBase-bom">
    ...
    <div class="right">
      ...
      <div :class="{ bottom: true, map: showType == 'map' }">
        <div class="bottom-body">
          <jvs-table v-if="showType == 'list'" :option="tableOption" :loading="tableLoading" :data="tableData" rowKey="jvsTableKey">
            <template #menu="scope">
              ...
              >
                <template #reference>
                  <el-button text size="small"
                    ><span style="color: #f56c6c">{{ t('table.delete') }}</span></el-button
                  >
                </template>
              </el-popconfirm>
            </template>
    ...
    <!-- 新增 修改 -->
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="52%" append-to-body :close-on-click-modal="false" :before-close="handleClose">
      <div v-if="dialogVisible" class="dialog-form-box">
        <jvs-form
          :ref="el => dynamicRefMap(el, 'materialForm')"
          :option="formOption"
          :formData="rowData"
          @init="initHandle"
          @submit="submitHandle"
    ...
    <!-- 选择父级物料编码 -->
    <el-dialog v-model="materialVisible" title="选择物料编码" width="52%" append-to-body :close-on-click-modal="false" :before-close="materialClose">
...
<script lang="ts" setup name="bom">
  ...
  function getListHandle(pageInfo?) {
    if (currentBom.value && currentBom.value['id']) {
      ...
      getBomTree(currentBom.value['id'])
        .then(res => {
          if (res.data && res.data.code == 0 && res.data.data) {
            mapData.value = {
              nodes: [],
  ...
  function getMaterialHandle(pageInfo?) {
    ...
    getMaterialList(Object.assign(queryParams.value, tp))
      .then(res => {
